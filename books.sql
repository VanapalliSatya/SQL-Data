...
SELECT REPLACE(CONCAT('I', ' ', 'LIKE', ' ', 'YOU'),' ','-');
...
SELECT UPPER(CONCAT(author_lname,author_fname)) FROM books;
...
SELECT title, CHAR_LENGTH(title) FROM books;
 ...
 SELECT CONCAT(SUBSTRING(title,1,10), '...'),author_fname,author_lname FROM books;
 
 SELECT CONCAT_WS(',',author_lname,author_fname) FROM books;
 
 SELECT CONCAT(stock_quantity, ' in stock ') FROM books;
